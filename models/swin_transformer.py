...
def compute_mask(D, H, W, window_size, shift_size, device):
    ...
    attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2) # nW, ws[0]*ws[1]*ws[2], ws[0]*ws[1]*ws[2]
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            x = self.downsample(x) # B D H/2 W/2 2C
...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(self, patch_size=(4, 4, 4), in_chans=3, embed_dim=96, depths=[2, 2, 6, 2],
        ...
        # spilt image into non-overlapping patches
        ...
        self.layer_num_features = [int(embed_dim * 2 ** i) for i in range(self.num_layers)]
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            x = layer(x.contiguous()) # B 2^iC D Wh/2^i Ww/2^i
